<div x-data="{ open: false }" class="relative" wire:poll.60s>
    <button @click="open = !open" type="button"
        class="relative p-3 text-ink-800 transition hover:text-accent focus:outline-none sm:p-2"
        ...
        @if ($unreadCount > 0)
            <span class="absolute -top-0.5 -end-0.5 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-accent px-1 text-[10px] font-semibold text-white">
    ...
    <div x-show="open" x-cloak @click.outside="open = false"
        ...
        class="absolute end-0 z-50 mt-2 w-80 max-w-[calc(100vw-2rem)] overflow-hidden rounded-xl border border-ink bg-white shadow-lg">
        <div class="flex items-center justify-between border-b-2 border-ink px-4 py-3">
            ...
            @if ($unreadCount > 0)
                <button wire:click="markAllRead" class="text-[11px] font-semibold uppercase tracking-label text-accent-700 transition hover:text-accent">
                    Mark all read
                </button>
            @endif
        </div>
            ...
            @forelse ($recent as $notification)
                <button wire:click="open('{{ $notification->id }}')" wire:key="bell-{{ $notification->id }}"
                    ...
                    <div class="flex items-start gap-2.5">
                        ...
                        @endunless
                        <div class="min-w-0">
                            <p class="truncate text-sm font-semibold text-ink">{{ $notification->data['title'] ?? '' }}</p>
        ...
        <a href="{{ route('notifications') }}" wire:navigate
            class="block border-t-2 border-ink px-4 py-3 text-center text-[11px] font-semibold uppercase tracking-label text-accent-700 transition hover:bg-ink-100">
            View all
        </a>
    </div>
</div>
